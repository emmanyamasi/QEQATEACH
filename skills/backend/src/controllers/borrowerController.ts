...
import { Borrow, BorrowRequest } from "../utils/types/borrowTypes";
import { Request,Response } from "express";


export const createBorrow = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    const { book_id, user_id } = req.body as BorrowRequest
    ...
    try {
        ...
        const newBorrow = await pool.query(
            `INSERT INTO public.borrowers (user_id, book_id, copy_id, librarian_id, borrow_date, return_date, status, created_at, updated_at)
        ...
        const borrow: Borrow = newBorrow.rows[0];
        res.status(201).json({
