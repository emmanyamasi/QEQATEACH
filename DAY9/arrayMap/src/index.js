//properties -id ,name ,location,friends ,posts
...
// Get the latest post date from all users
const latestPostDate = new Date(Math.max(
    ...users.flatMap(user => user.posts.map(post => new Date(post.timestamp)))
  ));
  
  // Calculate one week before the latest post date
  const oneWeekBeforeLatest = new Date(latestPostDate);
  oneWeekBeforeLatest.setDate(latestPostDate.getDate() - 7);
  
  console.log("Latest Post Date:", latestPostDate);
  console.log("One Week Before Latest:", oneWeekBeforeLatest);

  const activeUsers = users.filter(user => 
    user.posts.some(post => new Date(post.timestamp) > oneWeekBeforeLatest)
  );
...
const popularPosts = activeUsers.map(user => ({
    name :user.name,
    posts: user.posts.filter(post => post.likes >= 10)
